...
1. Определи, к какому **Domain** относится задача
2. Определи **тип изменения**: новый use-case, сценарий, правка существующего
3. Определи **change_class**: breaking / additive / compatible
4. Если задача затрагивает несколько use-case — декомпозируй на отдельные атомы
5. Покажи пользователю план атомов и получи подтверждение
